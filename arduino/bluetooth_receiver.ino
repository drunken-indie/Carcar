...
void loop()
{
  ...
  if(byte_count)//If there are any bytes then deal with them
  {
    ...
    int inDataInt=String(inData).toInt();
    left=inDataInt/8;
    inDataInt=inDataInt%8;
    right=inDataInt/4;
    inDataInt=inDataInt%4;
    down=inDataInt/2;
    inDataInt=inDataInt%2;
    up=inDataInt;
    writeUp(up);
    writeDown(down);
    writeRight(right);
